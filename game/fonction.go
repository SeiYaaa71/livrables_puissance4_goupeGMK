...
// ✅ RAJOUTÉ: Liste de messages d’encouragement
...
// Nouvelle partie
func NewGame() *Game {
	// rand.Seed(time.Now().UnixNano()) // Plus nécessaire depuis Go 1.20 (seed auto)
	...
	for row := Rows - 1; row >= 0; row-- {
		if g.Grid[row][col] == 0 {
			...
			if g.checkWin(row, col) {
				...
				// CORRIGÉ: Messages clairs pour le JavaScript de index.html
				if g.Winner == 1 {
			...
			// ✅ MODIFIÉ: Tirer un message d’encouragement aléatoire
